...
# import our parallel processing enngine for computing queries
# import engine.core as parallel


columns = ['GESTCEN', 'PRDTRACE', 'A_AGE',
           'A_SEX', 'ERN_VAL', 'PHIP_VAL', 'PMED_VAL']
...
for line in legend:
    ...
    # doesn't actually remove 'only' from all lines.
...
for row in range(0, len(table)):
    ...
    if table.at[row, "PHIP_VAL"] == 'Nan':
        ...
        table.at[row, "PHIP_VAL"] = str(
            round(int((table.at[row, "PHIP_VAL"])/12), 2))
    i += 1
print(i)


table = table.rename({'GESTCEN': 'StateCode', 'A_AGE': 'Age', 'A_SEX': 'Sex', 'PHIP_VAL': 'IndividualRate',
                     'PRDTRACE': 'Race', 'PMED_VAL': 'AmountPaidMedical', 'ERN_VAL': 'AnnualIncome'}, axis=1)


# making dataframe
rateDF = pd.read_csv("data/archive/Rate.csv",
                     usecols=['BusinessYear', 'Age', 'StateCode', 'IssuerId', 'IndividualRate'])
...
def create_test_data(data):
    ...
    for s in range(1, 200):
        ...
        print("Experimental dataset " +
              str(round((s/201), 4) * 100) + "% complete.")
